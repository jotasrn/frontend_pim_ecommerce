...
interface ModalPagamentoProps {
  aoFechar: () => void;
  aoSucessoCartao: (opcaoEntrega: OpcaoEntrega) => void;
  aoSucessoBoleto: (venda: Venda, opcaoEntrega: OpcaoEntrega) => void;
}

type Etapa = 'entrega' | 'pagamento' | 'confirmacao';
type MetodoPagamento = 'cartao' | 'boleto';
...
const ModalPagamento: React.FC<ModalPagamentoProps> = ({ aoFechar, aoSucessoCartao, aoSucessoBoleto }) => {
  ...
  useEffect(() => {
    if (!carregandoEnderecos && getValues('enderecoSelecionadoId') === undefined) {
      ...
      setValue('enderecoSelecionadoId', valorInicial);
    }
  ...
  const proximaEtapa = async () => {
    ...
    if (usuario && (!usuario.cpf || !usuario.telefone)) {
      showToast.error("Perfil incompleto. Complete seu CPF e Telefone em Minha Conta.");
      aoFechar();
    ...
    try {
      ...
      if (metodoPagamento === 'cartao') {
        ...
        vendaDTO = {
          ...
          enderecoEntregaId: (opcaoEntrega === 'entrega') ? idEnderecoParaApi : undefined,
        ...
        vendaDTO = {
          ...
          enderecoEntregaId: (opcaoEntrega === 'entrega' || metodoPagamento === 'boleto') ? idEnderecoParaApi : undefined,
          itens: itensDoCarrinho.map(item => ({ produtoId: item.id, quantidade: item.quantidade }))
        };
      }
      ...
      if (respostaVenda.statusPedido === "PAGAMENTO_APROVADO") {
        ...
        aoSucessoCartao(dadosFormulario.opcaoEntrega);
      }
      else if (respostaVenda.statusPedido === "AGUARDANDO_PAGAMENTO") {
        limparCarrinho();
        if (respostaVenda.boletoUrl) {
            aoSucessoBoleto(respostaVenda, dadosFormulario.opcaoEntrega);
        } else {
