...
const ModalLogin: React.FC<ModalLoginProps> = ({ onClose, onRegisterClick }) => {
  const [email, setEmail] = useState('');
  const [senha, setSenha] = useState('');
  const [mostrarSenha, setMostrarSenha] = useState(false);
  const [mensagemErro, setMensagemErro] = useState('');
  const [processandoLogin, setProcessandoLogin] = useState(false);
  const { login, loginComGoogle, carregando: carregandoAuth } = useAuth();
  ...
  const estaCarregando = processandoLogin || carregandoAuth;
      ...
      >
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={onClose} disabled={estaCarregando} className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 disabled:opacity-50 transition-colors">
              <X className="h-5 w-5" />
            </button>
          </div>
            ...
            <div>
              ...
              <div className="relative">
               ...
               <div className="text-right mt-1">
                   <button type="button" disabled={estaCarregando} className="text-xs text-green-600 hover:text-green-500 dark:text-green-400 dark:hover:text-green-300 font-medium disabled:opacity-50">Esqueceu a senha?</button>
               </div>
            </div>
